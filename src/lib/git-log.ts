...
/**
 * Gets git log as JSON array, properly escaping special characters
 * 
 * @deprecated Use getCommits() or getCommitsFromBranch() instead
 * @param options - Git log options (e.g., number of commits, branch, etc.)
 * @returns Array of GitCommit objects
 */
export function getGitLogAsJson(options: {
  maxCount?: number;
  branch?: string;
  since?: string;
  until?: string;
  ...
  if (options.branch) {
    return getCommitsFromBranch(options.branch, {
...
/**
 * Gets git log as a JSON string, properly formatted
 * 
 * @param options - Git log options
 * @param pretty - Whether to pretty-print the JSON (default: false)
 * @returns JSON string
 */
export function getGitLogAsJsonString(
  options: Parameters<typeof getGitLogAsJson>[0] = {},
  pretty: boolean = false
): string {
  const commits = getGitLogAsJson(options);
  return JSON.stringify(commits, null, pretty ? 2 : 0);
}
